...
export class PaymentService {
  ...
  getMyPayments(pagination?: { page: number; pageSize: number }): Observable<PagedResult<Payment>> {
    ...
    return this.http.get<PagedResult<Payment>>(`${environment.apiUrl}/payments/my${params}`);
  }
  ...
  getPaymentByBookingId(bookingId: string): Observable<Payment> {
    return this.http.get<Payment>(`${environment.apiUrl}/payments/booking/${bookingId}`);
  }

  getPaymentSummary(): Observable<PaymentSummary> {
    return this.http.get<PaymentSummary>(`${environment.apiUrl}/payments/summary`);
  }

  cancelPayment(id: string): Observable<Payment> {
    return this.http.put<Payment>(`${environment.apiUrl}/payments/${id}/cancel`, {});
  }
